...
export const MessageRegister = ({status, message,onClose}) => {
  ...
  return (
    <div 
      ...
      onClick={onClose}
    >
      <div className="cursor-default bg-white p-6 rounded-2xl max-w-[560px] flex flex-col items-center gap-y-4">
        <img src={status !== 'ERROR' ? checkIcon : errorIcon} className='w-[80px]' alt="Icon" />
        <p className='font-mont font-semibold text-[18px] text-center'>{message}</p>
        <button 
